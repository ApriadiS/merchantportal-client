...
function handleEditPromoClick(promo: PromoObject) {
  ...
  adminPromoDisplay = promo.adminPromoType === 'FIX' ? formatCurrencyInput(promo.adminPromo) : String(promo.adminPromo);
  isPromoFormVisible = true;
  window.scrollTo(0, 0);
}
function handleDeletePromo(id: string) {
  if (confirm('Apakah Anda yakin ingin menghapus promo ini?')) {
    setPromos(promos.filter((p) => p.idPromo !== id));
    resetPromoForm();
  }
}
function handlePromoInputChange(e: Event) {
  ...
  const isNumberField = ['tenorPromo', 'subsidiPromo', 'interestRate'].includes(id);
  newPromo = {
    ...newPromo,
...
function handlePromoFormSubmit(e: Event) {
  e.preventDefault();
  if (editingPromoId) {
    setPromos(promos.map(p => p.idPromo === editingPromoId ? { ...newPromo, idPromo: editingPromoId } as PromoObject : p));
  } else {
...
<div class="space-y-5">
  <div class="flex items-center justify-between mb-6 px-4 sm:px-0">
    ...
    <button type="button" aria-label="Tambah Promo" class="rounded-full bg-red-600 text-white p-2 shadow hover:bg-red-700 transition" on:click={() => isPromoFormVisible = true}>
  ...
  {#if isPromoFormVisible}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg relative max-h-[90vh] m-4 flex flex-col">
        <button class="absolute top-3 right-3 text-gray-400 hover:text-gray-700" aria-label="Tutup" on:click={resetPromoForm}>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
        <div class="overflow-y-auto p-6 sm:p-8 flex-1">
          <h3 class="text-xl font-bold text-gray-800 mb-6">{editingPromoId ? 'Edit Promo' : 'Buat Promo Baru'}</h3>
          <PromoForm
            promo={{
            ...
            stores={stores}
            minTransactionDisplay={minTransactionDisplay}
            adminPromoDisplay={adminPromoDisplay}
  ...
  <div class="mt-8">
    <div class="flex items-center gap-2 mb-4">
      {#if selectedPromos.length > 0}
        <input type="checkbox" id="selectAllPromo" checked={selectAll} on:change={toggleSelectAll} class="h-4 w-4 rounded border-gray-300" />
    ...
    {#each promos as promo}
      <PromoCard
        ...
        onToggleActive={(promoObj) => {
          ...
          setPromos(promos.map((p) => p.idPromo === promoObj.idPromo ? { ...promoObj } : p));
        }}

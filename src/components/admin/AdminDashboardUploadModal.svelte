...
onMount(async () => {
	console.log('[AdminDashboardUploadModal] onMount called', { uploadedFile, uploadMode });
	if (uploadedFile) {
		console.log('[AdminDashboardUploadModal] Mulai parsing file', uploadedFile.name);
		isParsing = true;
		Papa.parse(uploadedFile, {
			header: true,
			skipEmptyLines: true,
			complete: async (results: Papa.ParseResult<any>) => {
				console.log('[AdminDashboardUploadModal] Parsing selesai', { results });
				// Ambil kolom dari meta.fields
				const csvFieldsRaw = results.meta?.fields ?? [];
				const csvFields = csvFieldsRaw.map(f => f.toLowerCase().replace(/\s+/g, ''));
				let templateFieldsRaw: string[] = [];
				let templateFields: string[] = [];
				if (uploadMode === 'store') {
					templateFieldsRaw = ['Nama Toko','Perusahaan','Alamat','Route'];
					templateFields = templateFieldsRaw.map(f => f.toLowerCase().replace(/\s+/g, ''));
				} else if (uploadMode === 'promo') {
					templateFieldsRaw = ['Judul Promo','Minimal Transaksi','Tenor','Subsidi','Admin','Admin Type','Bunga','Voucher','Tanggal Mulai','Tanggal Selesai','Free Cicilan'];
					templateFields = templateFieldsRaw.map(f => f.toLowerCase().replace(/\s+/g, ''));
				} else if (uploadMode === 'store_promo') {
					templateFieldsRaw = ['Nama Toko','Perusahaan','Alamat','Route','Judul Promo','Minimal Transaksi','Tenor','Subsidi','Admin','Admin Type','Bunga','Voucher','Tanggal Mulai','Tanggal Selesai','Free Cicilan'];
					templateFields = templateFieldsRaw.map(f => f.toLowerCase().replace(/\s+/g, ''));
				}
				// Bandingkan jumlah dan nama kolom
				const isFormatValid = csvFields.length === templateFields.length && csvFields.every((f, i) => f === templateFields[i]);
				if (!isFormatValid) {
				...
				parseError = '';
				isParsing = false;
				console.log('[AdminDashboardUploadModal] Data hasil parsing', parsedData);
		...
		console.warn('[AdminDashboardUploadModal] Tidak ada file untuk diparsing');
	}
});
...
async function checkDuplicate(data: any[]) {
	...
	if (uploadMode === 'store' || uploadMode === 'store_promo') {
		const { data: stores, error: storeErr } = await supabase.from('store').select('name,route');
		...
		if (!stores || stores.length === 0) {
			for (const row of data) {
				...
				const namaToko = row['Nama Toko'] ?? row['name'] ?? row['nama_toko'] ?? '';
				const perusahaan = row['Perusahaan'] ?? row['company'] ?? row['Company'] ?? '';
				const alamat = row['Alamat'] ?? row['address'] ?? row['Address'] ?? '';
			...
			for (const row of data) {
				...
				const alamat = row['Alamat'] ?? row['address'] ?? row['Address'] ?? '';
				const route = row['Route'] ?? row['route'] ?? '';
				if ((route && storeRoutes.has(route)) || (namaToko && storeNames.has(namaToko))) {
					const dupObj = {
						'Nama Toko': namaToko,
						'Route': route,
						'Company': perusahaan,
	...
	if (uploadMode === 'promo' || uploadMode === 'store_promo') {
		const { data: promos, error: promoErr } = await supabase.from('promo').select('title_promo,voucher_code');
		if (promoErr) {
		...
		for (const row of data) {
			...
			const judulPromo = row['Judul Promo'] ?? row['title_promo'] ?? row['judul_promo'] ?? '';
			const tenor = row['Tenor'] ?? row['tenor'] ?? '';
			const voucher = row['Voucher'] ?? row['voucher_code'] ?? row['voucher'] ?? '';
			const tanggalSelesai = row['Tanggal Selesai'] ?? row['end_date_promo'] ?? row['tanggal_selesai'] ?? '';
			if ((judulPromo && promoTitles.has(judulPromo)) || (voucher && promoVouchers.has(voucher))) {
				duplicatePromo.push({
...
async function saveValidData() {
	...
	try {
		if (uploadMode === 'store') {
			...
			const { data: storeRes, error: storeErr } = await supabase.from('store').insert(storeInserts).select();
			...
			const { data: promoRes, error: promoErr } = await supabase.from('promo').insert(promoInserts).select();
			...
			if (storeRes && promoRes) {
				...
				const { error: relErr } = await supabase.from('promo_store').insert(promoStoreInserts);
...
{#if uploadedFile}
	{#if showFormatModal}
		<div class="fixed inset-0 z-60 flex items-center justify-center bg-black/40">
			<div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md flex flex-col items-center">
				<h4 class="text-lg font-bold text-red-700 mb-2">Format Salah</h4>
				<div class="text-sm text-gray-700 whitespace-pre-line mb-4">{formatError}</div>
				<button class="px-4 py-2 rounded bg-red-600 text-white font-semibold hover:bg-red-700 transition text-sm" on:click={() => showFormatModal = false}>Tutup</button>
			</div>
		</div>
	{:else}
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
			<div class="bg-white rounded-xl shadow-2xl p-8 w-full max-w-3xl flex flex-col">
				...
				{:else}
					<div class="mb-4 text-green-700 font-semibold">Data berhasil diparsing: {parsedData.length} baris</div>
					<!-- Collaps UI -->
					{#if uploadMode === 'store'}
						<CollapsMenu title="Data Store Valid" data={validStore} type="store" />
						<CollapsMenu title="Data Store Duplicate" data={duplicateStore} type="store" />
					{:else if uploadMode === 'promo'}
						<CollapsMenu title="Data Promo Valid" data={validPromo} type="promo" />
						<CollapsMenu title="Data Promo Duplicate" data={duplicatePromo} type="promo" />
					{:else if uploadMode === 'store_promo'}
						<CollapsMenu title="Data Store Valid" data={validStore} type="store" />
						<CollapsMenu title="Data Store Duplicate" data={duplicateStore} type="store" />
						<CollapsMenu title="Data Promo Valid" data={validPromo} type="promo" />
				...
				<div class="flex flex-col gap-2 mt-6">
					...
					<div class="flex justify-end gap-2">
						<button class="px-4 py-2 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 transition text-sm" on:click={saveValidData} disabled={isSaving}>
							{isSaving ? 'Menyimpan...' : 'Simpan Data Valid'}
						</button>
						<button class="px-4 py-2 rounded bg-gray-100 text-gray-700 font-semibold hover:bg-gray-200 transition text-sm" on:click={onClose} disabled={isSaving}>Tutup</button>
					</div>
				</div>
			</div>
		</div>

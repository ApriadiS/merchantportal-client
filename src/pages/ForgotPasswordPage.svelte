<script lang="ts">
  ...
  import { supabase } from "../lib/core/api/supabaseClient";
  ...
  async function handleEmailSubmit(e: Event) {
    ...
    if (error) {
      ...
      message =
        "Jika akun dengan email tersebut ada, kode OTP akan dikirimkan.";
      return;
    }
        ...
        .then(async ({ error }) => {
          ...
          try {
            ...
            if (!profileError && data && Array.isArray(data.roles)) {
              const rolesValid =
                data.roles.includes("mdo") && data.roles.includes("admin");
              if (!rolesValid) {
                log("Lupa password: roles tidak valid setelah OTP", {
                  email,
                  roles: data.roles,
                });
            ...
            log("Lupa password: error query profiles setelah OTP", {
              email,
              err,
            });
            window.location.href = "/admin";
          ...
          log(
            "Lupa password: OTP valid & roles valid, lanjut ke input password baru",
            { otp },
          );
          step = "password";
          message = null;
  ...
  async function handlePasswordSubmit(e: Event) {
    e.preventDefault();
    errorMsg = null;
    log("Lupa password: submit password baru", {
      newPassword,
      confirmPassword,
    });
    if (newPassword !== confirmPassword) {
      log("Lupa password: password tidak sama", {
        newPassword,
        confirmPassword,
      });
...
<main class="min-h-screen flex items-center justify-center bg-gray-50">
  <form
    class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-sm space-y-6"
    autocomplete="off"
    on:submit={step === "email"
      ? handleEmailSubmit
      : step === "password"
        ? handlePasswordSubmit
        : undefined}
  >
    <h1 class="text-2xl font-bold text-[#ff4745] mb-2 text-center">
      Lupa Password Admin
    </h1>
    {#if step === "email"}
      <p class="text-gray-600 text-sm text-center mb-4">
        Masukkan email admin Anda untuk reset password.
      </p>
      <input
        type="email"
        bind:value={email}
      ...
      <button
        type="submit"
        class="w-full py-3 rounded-md bg-[#ff4745] text-white font-bold hover:bg-red-600 transition"
        disabled={isLoading}
      >
        {#if isLoading}
          <svg
    ...
    {#if errorMsg}
      <div class="text-red-600 text-sm font-semibold text-center mt-2">
        {errorMsg}
      </div>
    {/if}
    {#if message}
      <div class="text-green-600 text-sm font-semibold text-center mt-2">
        {message}
      </div>
    {/if}
    <div class="mt-4 text-center">
      <a
        href="/admin"
        class="text-sm text-[#ff4745] hover:underline font-semibold"
        >Kembali ke login</a
      >
    </div>
  </form>

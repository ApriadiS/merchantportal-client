...
let store: StoreObject = { id: '', name: 'Toko Contoh', company: '', address: '', route: '' };
...
<div class="min-h-screen font-sans flex items-center justify-center p-4">
	<main class="bg-white rounded-2xl shadow-xl p-4 sm:p-8 w-full max-w-lg mx-auto">
		<header class="text-center">
			<img src="/akulaku.webp" alt="Akulaku Logo" class="h-12 mx-auto" />
			<h1 class="text-2xl font-bold text-[#ff4745] mt-4">Simulasi Cicilan - {store?.name}</h1>
			<p class="text-gray-500 text-sm mt-1">Hitung cicilan kredit barangmu dengan mudah</p>
		</header>
		...
		<form on:submit={handleCalculate} class="space-y-4">
			...
			<div class="bg-[#fff1f1] p-4 rounded-xl">
				<h2 class="text-center font-semibold text-[#ff4745] mb-4">Hasil Simulasi Cicilan</h2>
				   <div class="grid grid-cols-3 gap-2 sm:gap-4 text-center">
						{#each Object.keys(results).map(Number) as tenor (tenor)}
						   <div class="relative">
							   <div class="bg-white/70 p-3 rounded-lg flex flex-col items-center justify-center min-h-[100px]">
								   <p class="text-sm text-gray-600 h-6">{tenor} Bulan</p>
							 <p class="text-sm sm:text-base font-bold text-[#ff4745] my-1 whitespace-nowrap">
									{typeof results[tenor as number]?.monthlyInstallment === 'number'
										? formatCurrency(results[tenor as number]?.monthlyInstallment ?? 0)
										: '-'}
								   </p>
								   <button
									   type="button"
								   ...
								   >
										{#if expandedTenor === tenor}
										   <svg class="w-5 h-5 text-[#ff4745]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>
									   {:else}
				   ...
				   <div class="mt-4 p-5 bg-gray-100 rounded-xl text-sm text-gray-700 font-medium transition-all duration-300 ease-in-out">
					   <ul class="space-y-2">
						   ...
						   <li class="flex justify-between items-center text-base mt-2 pt-2 border-t border-gray-300">
							   ...
							   <span class="font-bold text-[#ff4745]">{formatCurrency(currentDetails.totalBill)}</span>
						   </li>
					   </ul>
		...
		<footer class="mt-6 text-center">
			...
			<p class="text-xs text-gray-500 mt-3 px-4 leading-relaxed"><b>Catatan:</b> Simulasi ini hanya perkiraan. Hubungi kami untuk detail promo dan kode voucher yang berlaku.</p>
			<a

  ...
  >
          ...
          {#each Object.keys(results).map(Number) as tenor (tenor)}
            <div class="relative">
              ...
              >
                ...
                >
                  {typeof results[tenor as number]?.monthlyInstallment ===
                  "number"
                    ? formatCurrency(
                        results[tenor as number]?.monthlyInstallment ?? 0,
                      )
                    : "-"}
                </p>
                <button
    ...
    <footer class="mt-6 text-center">
      ...
      <p class="text-xs text-gray-500 mt-3 px-4 leading-relaxed">
        <b>Catatan:</b> Simulasi ini hanya perkiraan. Hubungi kami untuk detail promo
        dan kode voucher yang berlaku.
      </p>
      <a

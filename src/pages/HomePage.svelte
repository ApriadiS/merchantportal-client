<script lang="ts">
  import { log, warn } from "../lib/core/utils/logger";
  log("HomePage loaded");
...
<div class="min-h-screen font-sans flex items-center justify-center p-4">
  <main
    class="bg-gray-50 rounded-2xl shadow-xl p-4 sm:p-8 w-full max-w-lg mx-auto"
  >
    <header class="text-center">
      <img src="/akulaku.png" alt="Akulaku Logo" class="h-12 mx-auto" />
      <h1 class="text-2xl font-bold text-red-500 mt-4 sr-only">
        Simulasi Cicilan Akulaku
      </h1>
    ...
    <form on:submit={handleCalculate} class="space-y-4">
      ...
      <div class="bg-[#fef1f1] p-4 rounded-xl">
        <h2 class="text-center font-semibold text-red-500 mb-4">
        ...
        <div class="grid grid-cols-3 gap-2 sm:gap-4">
          {#each TENORS as tenor}
            <div class="relative text-center">
              <div
                class="bg-[#fffbfa] p-3 rounded-lg flex flex-col items-center justify-center min-h-[100px]"
              >
                <p class="text-sm text-gray-600 h-6">{tenor} Bulan</p>
                <div
                  class="flex-grow flex items-center my-1 text-center justify-center w-full"
                >
                  <p
                    class="text-sm sm:text-base font-bold text-red-500 whitespace-nowrap"
                  >
                    {results[tenor]
                      ? formatCurrency(results[tenor].monthlyInstallment)
                      : "-"}
                  </p>
                </div>
                <button
    ...
    <footer class="mt-6 text-center">
      ...
      <p class="text-xs text-gray-500 mt-3 px-4 leading-relaxed">
        <b>Catatan:</b> Simulasi ini hanya perkiraan. Hubungi kami untuk detail lebih
        lanjut.
      </p>
      <a

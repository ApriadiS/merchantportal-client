<script lang="ts">
   ...
   import CollapsMenu from "../shared/CollapsMenu.svelte";
   ...
   onMount(async () => {
      ...
      if (uploadedFile) {
         ...
         Papa.parse(uploadedFile, {
            ...
            error: (err: Error) => {
               console.error(
                  "[AdminDashboardUploadModal] ERROR parsing file",
                  err
               );
      ...
      if (uploadMode === "store" || uploadMode === "store_promo") {
         ...
         if (!stores || stores.length === 0) {
            for (const row of data) {
               console.log(
                  "[checkDuplicate] DB kosong, row dianggap valid:",
                  row
               );
               const namaToko =
            ...
            for (const row of data) {
               ...
               if (
                  ...
                  console.log(
                     "[checkDuplicate] duplicateStore (added):",
                     dupObj
                  );
               } else {
                  const validObj = {
      ...
      >
         ...
         >
            ...
            {#if isParsing || isCheckingDuplicate}
               <div class="text-[#ff4745] font-semibold mb-4">
                  Memproses file...
               </div>
            {:else if parseError}
